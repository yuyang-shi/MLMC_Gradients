...
parser.add_argument("--test-and-visualize", action="store_true", help="Test model, visualize and exit instead of training.")
parser.add_argument("--test-verbose", action="store_true", help="Test model (verbose) and exit instead of training.")
parser.add_argument("--bias-var-test", action="store_true", help="Perform bias variance test and exit instead of training.")
parser.add_argument("--load-best", action="store_true", help="Load best checkpoint in testing instead of latest. ")
...
if args.resume is None:
    ...
    config = {
        **config,
        ...
        "load_latest": not args.load_best
    }

else:
    with open(Path(args.resume) / "config.json", "r") as f:
        config = json.load(f)
        config["load_latest"] = not args.load_best
...
if args.print_model:
    from ml_ll.experiment import print_model
    for c in grid:
        print_model(c)
    should_train = False

if args.print_num_params:
    from ml_ll.experiment import print_num_params
    for c in grid:
        print_num_params(c)
    should_train = False
...
if should_train or args.test:
    from ml_ll.experiment import train, print_test_metrics, print_test_metrics_verbose, perform_bias_var_test
    with contextlib.suppress(KeyboardInterrupt):
        for c in grid:
            c = {**c, "seed": int(time.time() * 1e6) % 2**32}
            if args.bias_var_test:
                perform_bias_var_test(config=c, resume_dir=args.resume)
            elif args.test_verbose:
                print_test_metrics_verbose(config=c, resume_dir=args.resume)
            elif args.test_and_visualize:
                print_test_metrics(config=c, resume_dir=args.resume, visualize=True)
            elif args.test:
                print_test_metrics(config=c, resume_dir=args.resume, visualize=False)
            else:
                c["load_latest"] = True
